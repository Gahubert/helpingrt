...
/*   Updated: 2017/06/21 18:48:37 by ghubert          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <rt.h>

void	ft_display(t_env_g *env)
{
	SDL_SetRenderDrawColor(env->sdl.rend, 255, 255, 255, 255);
	SDL_RenderClear(env->sdl.rend);
	//ft_browse_pixels(env);
	ft_thread(env);
	SDL_RenderCopy(env->sdl.rend, env->env1.sdl.draw, NULL, NULL);
	SDL_RenderCopy(env->sdl.rend, env->env2.sdl.draw, NULL, NULL);
	SDL_RenderCopy(env->sdl.rend, env->env3.sdl.draw, NULL, NULL);
	SDL_RenderCopy(env->sdl.rend, env->env4.sdl.draw, NULL, NULL);
